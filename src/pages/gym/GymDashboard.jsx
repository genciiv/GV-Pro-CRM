      ...
      <div className="g2">
        <div className="card">
          ...
          {(ciToday||[]).length===0 ? <Empty icon="🚪" title="Asnjë hyrje sot ende"/> : (
            <div>{(ciToday||[]).map((c,i)=>(
              <div key={c.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 16px',borderBottom:i<ciToday.length-1?'1px solid var(--border)':'none'}}>
                ...
                <span style={{fontSize:11,color:'var(--tx4)'}}>{fmtTime(c.checked_in_at)}</span>
              </div>
            ))}</div>
          )}
        </div>
          ...
          <div>
            <div className="card" style={{marginBottom:16}}>
              ...
              <div className="card-b">
                <div style={{border:'2px dashed var(--border)',borderRadius:16,padding:32,textAlign:'center',cursor:loading?'wait':'pointer',transition:'all .2s',marginBottom:14}}
                  onClick={simulate}
                  onMouseEnter={e=>{if(!loading)e.currentTarget.style.borderColor='var(--tx)'}}
                  onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border)'}>
                  <div style={{width:160,height:160,margin:'0 auto 20px',position:'relative',borderRadius:12,background:'var(--border)',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
                    ...
                    {['tl','tr','bl','br'].map(p=>(
                      <div key={p} style={{position:'absolute',width:22,height:22,
                        ...
                        borderColor:'var(--tx)',borderStyle:'solid',
                  ...
                  <div style={{fontSize:12,color:'var(--tx4)'}}>Në pajisje reale: kamera aktivizohet automatikisht</div>
                </div>
                  ...
                  result.success ? (
                    <div style={{background:'var(--grl)',border:'1px solid #bbf7d0',borderRadius:12,padding:18,display:'flex',alignItems:'center',gap:16}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:12,color:'var(--tx3)',marginTop:2}}>{result.plan_name} {result.days_remaining!=null?`• ${result.days_remaining} ditë mbeten`:''}</div>
                      </div>
                    </div>
                  ) : (
        ...
        <div className="g2">
          <div className="card">
            ...
            <div className="card-b">
              ...
              {searchResults.length===0 && search && <div style={{textAlign:'center',padding:'20px 0',color:'var(--tx4)',fontSize:13}}>Asnjë anëtar nuk u gjet</div>}
              {searchResults.map(m=>(
            ...
            <tbody>
              ...
              (history||[]).map(c=>(
                <tr key={c.id}>
                  ...
                  <td style={{fontSize:12,color:'var(--tx3)'}}>{fmtTime(c.checked_in_at)}</td>
                </tr>
              ))}
            </tbody>
          </table></div>
            ...
            <tbody>
              ...
              filtered.map(m=>(
                <tr key={m.id} style={{cursor:'pointer'}} onClick={()=>setProfile(m.id)}>
                  ...
                  <td style={{color:'var(--tx3)'}}>{m.phone||'—'}</td>
                  <td><span className="bdg bdg-gy">{m.plan_name||'Pa abonim'}</span></td>
                  <td><StatusBadge status={memberStatus(m)}/></td>
                  <td style={{fontSize:12,color:m.days_remaining<=3?'var(--rd)':'var(--tx3)'}}>{m.end_date?fmtDate(m.end_date):'—'}</td>
      ...
      {renew && (
        <Modal title="💳 Rinovim Abonoimi" onClose={()=>setRenew(null)} footer={
          ...
          <div style={{background:'var(--surface2)',borderRadius:10,padding:14,marginBottom:16}}>
            ...
            <div style={{fontSize:12,color:'var(--tx3)',marginTop:4}}>Plani aktual: {renew.plan_name||'Pa abonim'} • Skadon: {renew.end_date?fmtDate(renew.end_date):'—'}</div>
          </div>
          <div className="fgp">
...
function MemberProfile({ memberId, gymId, plans, onBack }) {
  ...
  return (
    <div className="page-in">
      ...
      <div className="card" style={{marginBottom:16}}>
        <div style={{padding:20,borderBottom:'1px solid var(--border)',display:'flex',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}>
          <Avatar color={m.avatar_color} name={m.full_name} size="lg"/>
          <div style={{flex:1,minWidth:200}}>
            <div style={{fontFamily:'Georgia,serif',fontSize:22,marginBottom:8}}>{m.full_name}</div>
            <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:12}}>
              <StatusBadge status={memberStatus(m)}/>
              {m.plan_name&&<span className="bdg bdg-gy">{m.plan_emoji} {m.plan_name}</span>}
              {m.phone&&<span style={{fontSize:12,color:'var(--tx3)'}}>📞 {m.phone}</span>}
              {m.email&&<span style={{fontSize:12,color:'var(--tx3)'}}>📧 {m.email}</span>}
              {m.birthday&&<span style={{fontSize:12,color:'var(--tx3)'}}>🎂 {fmtDate(m.birthday)}</span>}
              {m.gender&&<span style={{fontSize:12,color:'var(--tx3)'}}>{m.gender==='M'?'♂ Mashkull':'♀ Femër'}</span>}
            </div>
          ...
          <div style={{textAlign:'center',cursor:'pointer'}} onClick={()=>printQR(m.full_name, m.qr_code)} title="Kliko për të printuar">
            ...
            <div style={{fontSize:10,color:'var(--tx4)',marginTop:6}}>Kliko → Printo</div>
          </div>
        </div>

        <div style={{padding:'0 20px 20px'}}>
          <div style={{display:'flex',borderBottom:'1px solid var(--border)',marginTop:16,marginBottom:16}}>
            {[['overview','📊 Pasqyra'],['memberships','🎫 Abonimet'],['payments','💰 Pagesat'],['checkins','🚪 Check-ins']].map(([k,l])=>(
              <div key={k} style={{padding:'8px 16px',cursor:'pointer',fontSize:13,fontWeight:500,color:tab===k?'var(--tx)':'var(--tx3)',borderBottom:tab===k?'2px solid var(--tx)':'2px solid transparent',transition:'all .12s',whiteSpace:'nowrap'}} onClick={()=>setTab(k)}>{l}</div>
            ))}
          </div>
            ...
            <div>
              <div className="g3" style={{marginBottom:16}}>
                {[
                  ...
                  <div key={i} style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:10,padding:16,textAlign:'center'}}>
                    <div style={{fontFamily:'Georgia,serif',fontSize:24,marginBottom:4}}>{v}</div>
                    <div style={{fontSize:11,color:'var(--tx3)'}}>{l}</div>
                  </div>
                ))}
              </div>
              {m.notes&&<div style={{background:'var(--surface2)',borderRadius:8,padding:12,fontSize:13,color:'var(--tx2)'}}>📝 {m.notes}</div>}
            </div>
          )}
            ...
            <div>
              {(!mss||mss.length===0)?<Empty icon="🎫" title="Asnjë abonim"/>:(
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  {mss.map(ms=>(
                    <div key={ms.id} style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:10,padding:14,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
                      <div>
                        <div style={{fontWeight:600,fontSize:14}}>{ms.plan?.emoji} {ms.plan?.name} — {fmtNum(ms.price_paid)} L</div>
                        <div style={{fontSize:12,color:'var(--tx3)',marginTop:4}}>{ms.start_date} → {ms.end_date}</div>
                      </div>
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
            ...
            (!payments||payments.length===0)?<Empty icon="💰" title="Asnjë pagesë"/>:(
              <div className="tw"><table>
                ...
                <tbody>{payments.map(p=>(
                  <tr key={p.id}>
                    <td style={{fontFamily:'monospace',fontSize:11,color:'var(--tx4)'}}>{p.invoice_number}</td>
                    <td><span className="bdg bdg-gy">{p.membership?.plan?.name||'—'}</span></td>
                    <td style={{fontWeight:600}}>{fmtNum(p.amount)} L</td>
                    <td style={{color:'var(--tx3)'}}>💵 {p.method}</td>
                    <td style={{fontSize:12,color:'var(--tx3)'}}>{fmtDate(p.created_at)}</td>
              ...
              <div className="tw"><table>
                ...
                <tbody>{checkins.map(c=>(
                  <tr key={c.id}>
                    ...
                    <td style={{color:'var(--tx3)'}}>{fmtTime(c.checked_in_at)}</td>
                    <td><span className="bdg bdg-bl">{c.method==='qr'?'📷 QR':'⌨️ Manual'}</span></td>
                  </tr>
                ))}</tbody>
              </table></div>
    ...
    <div className="page-in">
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div className="card-b">
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:10}}>
            {(plans||[]).map(p=>(
              <div key={p.id} style={{background:'var(--surface2)',border:'1.5px solid var(--border)',borderRadius:12,padding:14,textAlign:'center',transition:'all .15s',cursor:'default'}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor='var(--tx)';e.currentTarget.style.boxShadow='var(--shm)'}}
                ...
                <div style={{fontFamily:'Georgia,serif',fontSize:20,marginBottom:2}}>{fmtNum(p.price)}</div>
                <div style={{fontSize:10,color:'var(--tx4)'}}>ALL / {p.duration_days} ditë</div>
              </div>
            ))}
          </div>
        </div>
              ...
              filtered.map(ms=>{
                ...
                return(
                  <tr key={ms.id}>
                    ...
                    <td style={{fontSize:12,color:'var(--tx3)'}}>{ms.start_date}</td>
                    <td style={{fontSize:12,color:ms.status==='active'&&(new Date(ms.end_date)-new Date())/(86400000)<7?'var(--rd)':'var(--tx3)'}}>{ms.end_date}</td>
      ...
      {renew&&(
        <Modal title="💳 Rinovim" onClose={()=>setRenew(null)} footer={
          ...
          <p style={{fontSize:13,marginBottom:14,color:'var(--tx2)'}}>Anëtari: <strong>{renew.member?.first_name} {renew.member?.last_name}</strong></p>
            ...
            {planStats.slice(0,6).map(p=>(
              <div key={p.id}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:5,fontWeight:500}}>
                  ...
                  <span style={{color:'var(--tx3)'}}>{p.count} anëtarë</span>
                </div>
                <div className="prog"><div className="pf" style={{width:`${Math.round(p.count/maxPlan*100)}%`,background:'var(--tx)'}}/></div>
              </div>
            ))}
        ...
        <div className="card">
          ...
          <div className="card-b" style={{display:'flex',gap:20,alignItems:'center',flexWrap:'wrap'}}>
            {total===0?<Empty icon="👥" title="Asnjë anëtar"/>:<>
              <div style={{flex:1}}>
                {[['♂ Mashkull',genderM,'var(--bl)'],['♀ Femër',genderF,'var(--rd)']].map(([l,n,c])=>(
                  <div key={l} style={{marginBottom:12}}>
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:5}}><span style={{fontWeight:500}}>{l}</span><span style={{color:'var(--tx3)'}}>{n} ({total>0?Math.round(n/total*100):0}%)</span></div>
                    <div className="prog"><div className="pf" style={{width:`${total>0?Math.round(n/total*100):0}%`,background:c}}/></div>
                  </div>
                ))}
              </div>
              <div style={{textAlign:'center'}}>
                <div style={{fontFamily:'Georgia,serif',fontSize:36,fontWeight:900}}>{total}</div>
                <div style={{fontSize:12,color:'var(--tx3)'}}>Total</div>
              </div>
            </>}
          </div>
        </div>
        <div className="card">
          ...
          <div className="card-b">
            {[
              ...
              ['Frozen',      (members||[]).filter(m=>memberStatus(m)==='frozen').length,   'var(--bl)'],
              ['Pa Abonim',   (members||[]).filter(m=>memberStatus(m)==='none').length,     'var(--tx4)'],
            ].map(([l,n,c])=>(
              <div key={l} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid var(--border)'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <span style={{fontSize:11,color:'var(--tx4)',width:36,textAlign:'right'}}>{total>0?Math.round(n/total*100):0}%</span>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="page-in">
      ...
      <div className="g2">
          ...
          <div className="card">
            ...
            <div className="card-b">
              <div style={{background:'var(--surface2)',borderRadius:8,padding:12,fontSize:13}}>
                <div style={{marginBottom:8}}><span style={{color:'var(--tx3)'}}>Email:</span> <strong>{form.email}</strong></div>
                <div style={{fontSize:12,color:'var(--tx4)'}}>Për të ndryshuar fjalëkalimin, kontakto administratorin e platformës.</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-hd"><div className="card-t">💰 Çmimet e Planeve</div><span style={{fontSize:11,color:'var(--tx4)'}}>Ndrysho çmimin dhe kliko Enter</span></div>
          <div className="card-b" style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            (plans||[]).map(p=>(
              <div key={p.id} style={{display:'flex',alignItems:'center',gap:12,padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
                ...
                <span style={{fontSize:11,color:'var(--tx4)',minWidth:40}}>{p.duration_days}d</span>
                <input
                  type="number"
                ...
                />
                <span style={{fontSize:12,color:'var(--tx4)',width:20}}>L</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <main className="main">
        <div className="topbar">
          ...
          <div className="tbr">
            ...
            <a href={`/book/${gymSlug}`} target="_blank" style={{fontSize:11,color:"var(--pu)",fontWeight:600,textDecoration:"none",background:"var(--pul)",padding:"4px 10px",borderRadius:20,border:"1px solid var(--pum)"}}>🔗 Booking Link</a>
