      ...
      <div className="card">
        ...
        {loading?<Loading/>:(appts||[]).length===0?(
          ...
          <div className="tw"><table>
            <thead><tr><th>Ora</th><th>Klienti</th><th>Stilisti</th><th>Shërbimi</th><th>Çmimi</th><th>Statusi</th><th>Veprime</th></tr></thead>
            <tbody>
              {(appts||[]).map(a=>(
                <tr key={a.id}>
  ...
  return (
    <div className="page-in">
      ...
      {step===1&&(
        <div className="g2">
          <div>
            <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>👤 Zgjidh Stilistin</div>
      ...
      {step===1&&(
        <div style={{marginTop:16,display:'flex',justifyContent:'flex-end'}}>
          <button className="btn btn-p" onClick={()=>{
            if(!selStaff){toast.error('Zgjidh stilistin');return}
  ...
  return (
    <div className="page-in">
      ...
      {loading?<Loading/>:(
        <div className="card">
          <div className="tw"><table>
            <thead><tr><th>Data & Ora</th><th>Klienti</th><th>Stilisti</th><th>Shërbimi</th><th>Çmimi</th><th>Pagesa</th><th>Statusi</th><th>Veprime</th></tr></thead>
            <tbody>
...
function StaffPage({ gymId }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        toast.success('✅ Stilisti u shtua!')
      }
  ...
  return (
    <div className="page-in">
      <div className="ph">
        <div><div className="pt">Stafi / Stilistët</div><div className="ps">{(staff||[]).length} stilistë</div></div>
        <button className="btn btn-p" onClick={()=>{setEditing(null);setForm({name:'',phone:'',email:'',speciality:'',start_time:'09:00',end_time:'19:00',slot_minutes:30,working_days:['Mon','Tue','Wed','Thu','Fri','Sat']});setShowAdd(true)}}>+ Shto Stilist</button>
      </div>
      {loading?<Loading/>:(
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
          {(staff||[]).length===0?<Empty icon="👤" title="Asnjë stilist" sub="Shto stilisten e parë"/>:
          (staff||[]).map(s=>(
      ...
      {showAdd&&(
        <Modal title={editing?`✏️ Edito — ${editing.name}`:'👤 Stilist i Ri'} onClose={()=>{setShowAdd(false);setEditing(null)}} footer={
          ...
          <div className="fg"><div className="fgp"><label>Emri *</label><input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Artan Stilist"/></div></div>
          <div className="fg c2">
            <div className="fgp"><label>Telefon</label><input value={form.phone} onChange={e=>set('phone',e.target.value)}/></div>
            <div className="fgp"><label>Specializimi</label><input value={form.speciality} onChange={e=>set('speciality',e.target.value)} placeholder="Stilist, Stilist..."/></div>
          </div>
          <div className="fg c2">
...
function ServicesPage({ gymId }) {
  ...
  const EMOJIS = ['✂️','🎨','💅','💄','🧖','💇','✨','🌸','🪄','💆','🦶','🧴','⚡','🌺']
  const CATS   = ['haircut','color','styling','nails','makeup','facial','waxing','eyebrows','massage','other']
  ...
  const addDefaults = async () => {
    const defaults = [
      { gym_id:gymId, name:'Prerje & Fëndërim', emoji:'✂️', duration_min:45, price:1500, category:'haircut', sort_order:1 },
      { gym_id:gymId, name:'Ngjyrosje e Plotë', emoji:'🎨', duration_min:120, price:4000, category:'color', sort_order:2 },
      { gym_id:gymId, name:'Highlights / Balayage', emoji:'✨', duration_min:150, price:6000, category:'color', sort_order:3 },
      { gym_id:gymId, name:'Kërbaçim / Drejtim', emoji:'💇', duration_min:60, price:2000, category:'styling', sort_order:4 },
      { gym_id:gymId, name:'Manikyr', emoji:'💅', duration_min:45, price:1200, category:'nails', sort_order:5 },
      { gym_id:gymId, name:'Pedikyr', emoji:'🦶', duration_min:60, price:1500, category:'nails', sort_order:6 },
      { gym_id:gymId, name:'Makeup', emoji:'💄', duration_min:60, price:3000, category:'makeup', sort_order:7 },
      { gym_id:gymId, name:'Trajtim Fytyre', emoji:'🧖', duration_min:60, price:2500, category:'facial', sort_order:8 },
    ]
...
const TITLES = { dashboard:'Dashboard', 'new-appointment':'Rezervim i Ri', appointments:'Rezervimet', calendar:'Kalendari', analytics:'Analytics', reminders:'🔔 Kujtesat', affiliate:'Affiliate', members:'Klientet', payments:'Pagesat', staff:'Stafi / Stilistët', services:'Sherbimet' }
    ...
    <div className="app">
      ...
      <aside className={`sidebar ${sbOpen?'open':''}`}>
        <div className="sb-logo">
          ...
          <div><div className="sb-name">{gymName}</div><div className="sb-sub">Vaqo Salon</div></div>
        </div>
        <nav className="nav">
          {NAV.map(s=>(
