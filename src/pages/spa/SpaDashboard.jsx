  ...
  return (
    <div className="page-in">
      ...
      <div className="g2" style={{marginBottom:14}}>
        <div className="card">
          <div className="card-hd"><div className="card-t">Trajtimie Disponueshme</div><button className="btn btn-s btn-sm" onClick={()=>setPage('services')}>Shiko</button></div>
          <div className="card-b">
            {[['💆','Masazh Relaksues','60-90 min'],['🌊','Hammam','45 min'],['🌿','Aromaterapi','75 min'],['💎','Trajtim Fytyre','60 min'],['🦶','Refleksologji','45 min'],['✨','Trajtim Trupi','90 min']].map(([ico,l,d])=>(
              <div key={l} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{l}</div><div style={{fontSize:11,color:'var(--tx4)'}}>{d}</div></div>
        ...
        <div className="card">
          ...
          <div className="card-b" style={{display:'flex',flexDirection:'column',gap:8}}>
            ...
            <div style={{background:'var(--pul)',borderRadius:9,padding:'10px',fontSize:12,color:'var(--pu)',fontWeight:600,textAlign:'center'}}>Gift Cards - Se shpejti</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        {loading?<Loading/>:(appts||[]).length===0?(
          ...
          <div className="tw"><table>
            <thead><tr><th>Ora</th><th>Klienti</th><th>Terapisti</th><th>Shërbimi</th><th>Çmimi</th><th>Statusi</th><th>Veprime</th></tr></thead>
            <tbody>
              {(appts||[]).map(a=>(
                <tr key={a.id}>
  ...
  return (
    <div className="page-in">
      ...
      <div style={{display:'flex',gap:0,marginBottom:24,background:'var(--border)',borderRadius:12,padding:4}}>
        {[['1','Terapisti & Shërbimi'],['2','Data & Ora'],['3','Klienti']].map(([n,l],i)=>(
      ...
      {step===1&&(
        <div className="g2">
          <div>
            <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>👤 Zgjidh Terapistin</div>
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              {(staff||[]).length===0?<Empty icon="👤" title="Asnjë terapist" sub="Shto terapist nga Konfigurimet"/>:
              (staff||[]).map(s=>(
                <div key={s.id} style={{display:'flex',alignItems:'center',gap:14,padding:'14px 16px',borderRadius:12,border:`2px solid ${selStaff?.id===s.id?'var(--tx)':'var(--border)'}`,background:selStaff?.id===s.id?'var(--surface2)':'#fff',cursor:'pointer',transition:'all .15s'}}
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:'var(--tx3)',marginTop:2}}>{s.speciality||'Terapiste'}</div>
      ...
      {step===1&&(
        <div style={{marginTop:16,display:'flex',justifyContent:'flex-end'}}>
          <button className="btn btn-p" onClick={()=>{
            if(!selStaff){toast.error('Zgjidh terapistin');return}
  ...
  return (
    <div className="page-in">
      ...
      {loading?<Loading/>:(
        <div className="card">
          <div className="tw"><table>
            <thead><tr><th>Data & Ora</th><th>Klienti</th><th>Terapisti</th><th>Shërbimi</th><th>Çmimi</th><th>Pagesa</th><th>Statusi</th><th>Veprime</th></tr></thead>
            <tbody>
...
function StaffPage({ gymId }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        toast.success('✅ Terapisti u shtua!')
      }
  ...
  return (
    <div className="page-in">
      <div className="ph">
        <div><div className="pt">Stafi / Terapistët</div><div className="ps">{(staff||[]).length} terapistë</div></div>
        <button className="btn btn-p" onClick={()=>{setEditing(null);setForm({name:'',phone:'',email:'',speciality:'',start_time:'09:00',end_time:'19:00',slot_minutes:30,working_days:['Mon','Tue','Wed','Thu','Fri','Sat']});setShowAdd(true)}}>+ Shto Terapist</button>
      </div>
      {loading?<Loading/>:(
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:16}}>
          {(staff||[]).length===0?<Empty icon="👤" title="Asnjë terapist" sub="Shto terapistin e parë"/>:
          (staff||[]).map(s=>(
            <div key={s.id} className="card" style={{padding:20}}>
              <div style={{display:'flex',gap:14,alignItems:'flex-start',marginBottom:16}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:12,color:'var(--tx3)',marginBottom:6}}>{s.speciality||'Terapiste'}</div>
      ...
      {showAdd&&(
        <Modal title={editing?`✏️ Edito — ${editing.name}`:'👤 Terapist i Ri'} onClose={()=>{setShowAdd(false);setEditing(null)}} footer={
          ...
          <div className="fg"><div className="fgp"><label>Emri *</label><input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Artan Terapist"/></div></div>
          <div className="fg c2">
            <div className="fgp"><label>Telefon</label><input value={form.phone} onChange={e=>set('phone',e.target.value)}/></div>
            <div className="fgp"><label>Specializimi</label><input value={form.speciality} onChange={e=>set('speciality',e.target.value)} placeholder="Terapist, Stilist..."/></div>
          </div>
          <div className="fg c2">
...
function ServicesPage({ gymId }) {
  ...
  const EMOJIS = ['💆','💆‍♀️','🧖','✨','🌿','🌸','🛁','🪨','💎','🌺','🧴','💪','🌊','☯️']
  const CATS   = ['massage','facial','body','hammam','waxing','aromatherapy','reflexology','hot_stone','other']
  ...
  const addDefaults = async () => {
    const defaults = [
      { gym_id:gymId, name:'Masazh Relaksues', emoji:'💆', duration_min:60, price:3000, category:'massage', sort_order:1 },
      { gym_id:gymId, name:'Masazh i Thellë', emoji:'💪', duration_min:60, price:3500, category:'massage', sort_order:2 },
      { gym_id:gymId, name:'Masazh Çiftesh', emoji:'👫', duration_min:60, price:6000, category:'massage', sort_order:3 },
      { gym_id:gymId, name:'Trajtim Fytyre', emoji:'🧖', duration_min:60, price:2500, category:'facial', sort_order:4 },
      { gym_id:gymId, name:'Trajtim Trupi', emoji:'✨', duration_min:90, price:4000, category:'body', sort_order:5 },
      { gym_id:gymId, name:'Hammam / Banjë Turke', emoji:'🛁', duration_min:45, price:2000, category:'hammam', sort_order:6 },
      { gym_id:gymId, name:'Depilim me Dyllë', emoji:'🌸', duration_min:30, price:1500, category:'waxing', sort_order:7 },
      { gym_id:gymId, name:'Aromaterapi', emoji:'🌿', duration_min:75, price:3500, category:'aromatherapy', sort_order:8 },
    ]
...
export default function SpaDashboard() {
  ...
  const gymName  = profile?.gym?.name || 'Spa & Masazh'
    ...
    <div className="app">
      ...
      <aside className={`sidebar ${sbOpen?'open':''}`}>
        <div className="sb-logo">
          ...
          <div><div className="sb-name">{gymName}</div><div className="sb-sub">Vaqo Spa</div></div>
        </div>
        <nav className="nav">
          {NAV.map(s=>(
